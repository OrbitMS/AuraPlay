...
export const VideoPlayer: React.FC<Props> = ({ video, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col animate-np-in" style={{ background: 'rgba(6,7,10,0.92)', backdropFilter: 'blur(20px)' }}>
      {/* Header */}
      ...
      <div className="flex-1 min-h-0 flex items-center justify-center px-6 pb-8">
        <div className="relative w-full mx-auto rounded-[14px] overflow-hidden"
          style={{ maxWidth: 'min(100%, calc((100vh - 160px) * 1.7778))', aspectRatio: '16 / 9', background: '#000', boxShadow: '0 30px 90px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.08)' }}>
          {error ? (
            ...
            <video ref={videoRef} src={url} controls autoPlay playsInline className="w-full h-full" style={{ background: '#000' }} />
          )}
        </div>
      </div>
    </div>
